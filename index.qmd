...
```{python}
# melt
emissions_long = table_df.melt(id_vars=['country'], var_name='year', value_name='emissions')
...
```{python}
# Convert emissions to numeric, handling dashes
emissions_long["emissions"] = pd.to_numeric(emissions_long["emissions"].astype(str).str.replace(r"[^\d.-]", "", regex=True))

show(emissions_long)

emissions_long["country"].value_counts()
```

```{python}
# Filter for years between 1990 and 2022
emissions_long_1990_2022 = emissions_long.query("1990 <= year <= 2022")

emissions_long_1990_2022 
#emissions_long_1990_2022["country"].value_counts()
```
```{python}
# Filter for specific countries
emissions_long_subset = emissions_long_1990_2022.query("country.isin(['Afghanistan', 'Qatar', 'Malaysia', 'Namibia', 'Niger'])")

emissions_long_subset
```
```{python}
# Sort for better visualization
emissions_long_subset = emissions_long_subset.sort_values(by=['country', 'year'])
emissions_long_subset
```
...
fig_chart = px.line(emissions_long_subset, x='year', y='emissions', color='country', 
                    title='Changes in the CO2 emissions by country with year', markers=True)

# Set y-axis range
fig_chart.update_yaxes(range=[0, 300])
...
#fig_chart = px.line(emissions_long_subset, x='year', y='emissions', color='country', title='Changes in the CO2 emissions<br>by country with year',markers=True)
